# 需求对接流转规范

> **适用阶段**：全阶段 | **优先级**：高 | **负责人**：周八
>
> 本文档定义美术 vs 策划需求对接全流程，包含需求单模板、评审机制、变更管控与回溯追踪。

---

## 1. 需求对接全流程

### 1.1 流程全景图

```
策划提需求 → 需求评审 → APM排期 → 美术制作 → 内部验收 → 策划验收 → 归档
    │            │           │          │           │           │         │
    ▼            ▼           ▼          ▼           ▼           ▼         ▼
 需求单模板  评审会决议  排期计划表  WIP进度     主美审核    策划确认   完成归档
 参考图上传  可行性评估  资源分配   中间产出    TA技术审核   修改意见   数据统计
```

### 1.2 各阶段标准流转时间

| 阶段 | 标准时效 | 超时预警 |
|------|---------|---------|
| 需求单填写 | 策划提交后 0.5d | — |
| 需求评审 | 收到后 1~2d | 2d 未评审自动催 |
| APM 排期 | 评审后 1d | — |
| 美术制作 | 按排期 | 燃尽图跟踪 |
| 内部验收 | 提交后 1d | 超时升级 |
| 策划验收 | 提交后 2d | 超时视为通过 |

---

## 2. 标准需求单模板

### 2.1 需求单字段定义

| 字段 | 必填 | 说明 | 示例 |
|------|------|------|------|
| **需求 ID** | ✅ | 唯一编号，APM 分配 | REQ-2026-0108 |
| **需求标题** | ✅ | 一句话描述 | 新英雄「夜叉」角色设计 |
| **需求类型** | ✅ | 原画/建模/UI/特效/动画/场景 | 角色原画 |
| **优先级** | ✅ | P0/P1/P2/P3 | P1 |
| **关联版本** | ✅ | 目标里程碑 | Alpha |
| **策划负责人** | ✅ | 需求发起人 | 王策划 |
| **美术负责人** | ✅ | APM 指定 | 张美术 |
| **需求描述** | ✅ | 详细的文字描述 | (详见下方) |
| **参考图** | ✅ | 至少 3 张参考图 | 上传至附件 |
| **验收标准** | ✅ | 量化的验收条件 | (详见下方) |
| **预估工期** | ⚠️ | 美术侧评估后填写 | 5d |
| **截止日期** | ✅ | 与里程碑对齐 | 2026-04-20 |
| **备注** | ❌ | 补充说明 | — |

### 2.2 优先级定义

| 等级 | 定义 | 响应时效 | 排期位置 |
|------|------|---------|---------|
| 🔴 **P0 紧急** | 阻塞里程碑 / Boss 要求 | 当天响应 | 插入当前 Sprint |
| 🟡 **P1 高** | 当前版本必须 | 1d 内响应 | 当前/下个 Sprint |
| 🟢 **P2 中** | 重要但不紧急 | 3d 内响应 | 正常排期 |
| ⚪ **P3 低** | 锦上添花 | 下版本处理 | 需求池候选 |

### 2.3 参考图规格要求

| 要求 | 标准 |
|------|------|
| **数量** | 至少 3 张（正面/侧面/细节或风格参考） |
| **分辨率** | ≥ 720p，清晰可辨别细节 |
| **标注** | 需标注"参考哪里"（颜色/造型/材质/风格） |
| **来源** | 注明原画出处，避免版权风险 |
| **反例** | 可提供"不要的风格"反面参考 |

---

## 3. 评审机制

### 3.1 评审会设置

| 项目 | 标准 |
|------|------|
| **频率** | 每周 1~2 次（周一/周四下午） |
| **时长** | 30~60 分钟 |
| **参与人** | APM（主持）、主美、对应工种组长、策划负责人 |
| **输出** | 评审记录表（通过/修改/拒绝/挂起） |

### 3.2 评审决策流程

```
策划讲述需求 → 美术评估可行性 → 讨论
                                  │
                    ┌──────────────┼──────────────┐
                    ▼              ▼              ▼
               ✅ 通过        🟡 修改后通过    ❌ 拒绝
               (直接排期)    (策划修改需求单   (说明原因
                             重新提交)         给替代方案)
```

### 3.3 评审检查要点

| 维度 | 问题 |
|------|------|
| **需求完整性** | 描述是否清晰？参考图是否充分？ |
| **技术可行性** | 现有管线能否实现？性能预算够吗？ |
| **工期合理性** | 截止日期是否留有 Buffer？ |
| **优先级合理性** | P0/P1 是否真的紧急？是否有挤占风险？ |
| **重复性检查** | 是否与已有需求重复？可否复用？ |

### 3.4 分歧处理规则

1. **美术与策划分歧** → APM 主持协商，给出数据支撑
2. **无法达成一致** → 升级到主美 + 制作人层面决策
3. **技术不可行** → TA 出具技术评估报告
4. **工期分歧** → APM 出具历史数据对比

---

## 4. 变更管控

### 4.1 变更申请流程

```
策划提出变更 → 填写变更申请单 → APM 影响评估
    │                                │
    │                     ┌──────────┼──────────┐
    │                     ▼          ▼          ▼
    │               影响小(<2d)  影响中(2~5d)  影响大(>5d)
    │                  │           │            │
    │               APM审批    主美+APM审批  制作人审批
    │                  │           │            │
    │                  └─────┬─────┘            │
    │                        ▼                  ▼
    │                   排期调整           正式变更会议
    └─────────────────→ 执行变更
```

### 4.2 变更影响评估模板

| 评估项 | 内容 |
|--------|------|
| **变更描述** | 具体变更了什么？ |
| **影响范围** | 涉及哪些已完成/进行中的资产？ |
| **工期影响** | 额外需要多少人天？ |
| **成本影响** | 额外费用（外包返工/加班）？ |
| **质量影响** | 是否影响其他资产的一致性？ |
| **里程碑影响** | 是否导致里程碑延期？ |
| **风险等级** | 低/中/高 |

### 4.3 变更冻结期规则

| 阶段 | 变更权限 |
|------|---------|
| 预研期 | ✅ 自由变更 |
| Alpha 前 2 周 | 🟡 P0 变更需审批 |
| Beta 前 1 月 | 🔴 仅 P0 变更，必须制作人审批 |
| RC 阶段 | 🚫 原则上禁止美术变更，仅修 Bug |

---

## 5. 常见冲突场景与解法

### 5.1 需求描述不清

| 项目 | 说明 |
|------|------|
| **表现** | "做一个好看的角色"、"参考某游戏的感觉" |
| **根因** | 策划缺乏美术语言，参考图不足 |
| **APM 解法** | 主动要求补充参考图，组织 15min 面对面沟通，产出一份美术理解确认书 |
| **预防** | 提供需求单填写指南培训 |

### 5.2 策划频繁变更

| 项目 | 说明 |
|------|------|
| **表现** | 原画做到一半改设定，建模快完成改造型 |
| **根因** | 策划设计未收敛就下需求 / 上级反复审 |
| **APM 解法** | 建立变更流程，量化变更成本，定期汇报变更次数和代价 |
| **话术** | "这次变更预计影响 3d 工期和 ¥5k 外包成本，需要您和制作人确认" |

### 5.3 优先级争议

| 项目 | 说明 |
|------|------|
| **表现** | 所有需求都标 P0，策划 A 说自己更急 |
| **根因** | 缺乏统一的优先级仲裁机制 |
| **APM 解法** | 建立优先级矩阵（紧急×重要），每周制作人会上统一排序 |
| **原则** | P0 全项目不超过 3 个，由制作人/总监拍板 |

---

## 附录：跨部门协作日历

| 时间 | 事项 | 参与人 |
|------|------|--------|
| 每周一 10:00 | 需求评审会 | APM + 策划 + 主美 |
| 每周三 14:00 | 美术内部 Review | APM + 各组长 |
| 每周五 16:00 | 周报同步 | APM → 策划/制作人 |
| Sprint 首日 | Sprint 规划 | 全美术团队 |
| Sprint 末日 | Sprint 评审+回顾 | 全美术团队 + 策划 |
