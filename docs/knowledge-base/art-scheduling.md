...
│ 📋 任务卡片 - [TASK-ID]                      │
├─────────────────────────────────────────────┤
│ 标题:   CH_Hero_Luna_Model_v2               │
│ 类型:   ☐原画 ☐建模 ☐贴图 ☐绑定 ☐特效 ☐动画 ☐UI │
│ 优先级: 🔴P0 / 🟡P1 / 🟢P2                    │
│ 工种:   角色组                                │
│ 负责人: 张三                                  │
│ 预估人天: 5d                                  │
...
```
发现偏差 → 排查原因（需求变更？人力不足？技术阻塞？）
         → 调整排期（压缩非关键路径 / 申请加人 / 降低优先级）
         → 通知干系人（主美 + 组长）
         → 48h 后复查
```
...
```
确认风险 → 召集紧急会议（APM + 主美 + 制作人）
         → 提出方案（A:砍需求 / B:加外包 / C:延期）
         → 制作人决策
         → 当天更新排期表
         → 每日跟踪直到恢复绿色
```
