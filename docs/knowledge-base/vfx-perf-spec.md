...
渲染顺序（从后到前）:
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Layer 0: 场景底层特效（地面裂缝/水面波纹）
Layer 1: 场景环境特效（落叶/雨雪/雾气）
Layer 2: 角色底层特效（脚底光环/站立印记）
Layer 3: 角色身体特效（Buff/Aura/附魔）
Layer 4: 角色技能特效（释放/飞行/爆炸）
Layer 5: 命中特效（Hit/受击反馈）
Layer 6: 前景特效（全屏闪白/镜头震动）
Layer 7: UI 特效（按钮光效/升级动画）
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
...
**解法**：
- 减少粒子层数，用贴图细节代替粒子数量
- 使用 Additive 混合代替 Alpha Blend（不产生排序开销）
- 远距离自动降低粒子数量
...
**解法**：
- 限制半透明面片最大不超过屏幕 1/4
- 使用 Mask 裁切透明区域（减少无效像素填充）
- 全屏效果优先用 Post-Process 实现
