...
> **[P0 事故]** 一旦发生可能导致他人数天工作白费。

| 📌 项目 | 📖 说明 |
|:---:|:---:|
| **现象** | A 提交覆盖了 B 的修改 |
| **原因** | 未使用文件锁定 / 提交前未 Update |
| **预防** | PSD/MAX 等二进制文件强制 `exclusive lock` |
...
|:---:|:---:|
| **现象** | 引擎中资产引用找不到文件 |
| **原因** | 本地 rename 后重新 add，而非 `svn move` |
| **预防** | 使用 `svn move` / `p4 move` 保留历史；CI 检查路径引用完整性 |
...
| 📌 项目 | 📖 说明 |
|:---:|:---:|
| **现象** | 提交了 2GB 的视频参考文件，仓库暴涨 |
| **原因** | `_Reference` 目录权限未限制 |
| **预防** | Pre-commit Hook 限制单文件大小（如 ≤ **200MB**） |
