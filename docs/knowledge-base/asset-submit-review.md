...
- ✅ [**资产提交前自检 Checklist**](#1️⃣-资产提交前自检-checklist)
  - [通用自检项](#11-通用自检项)
  - [角色资产专项](#12-角色资产专项)
  - [UI 资产专项](#13-ui-资产专项)
  - [特效资产专项](#14-特效资产专项)
- 📋 [**提交流程标准化**](#2️⃣-提交流程标准化)
  - [流程全景图](#21-流程全景图)
  - [详细步骤](#22-详细步骤)
- ⚙️ [**自动化审核流水线设计**](#3️⃣-自动化审核流水线设计)
  - [架构概览](#31-架构概览)
  - [命名检查脚本示例](#32-命名检查脚本示例-python)
  - [资产规格检查要点](#33-资产规格检查要点)
- 🔍 [**Art Review 机制**](#4️⃣-art-review-机制)
  - [审核角色与职责](#41-审核角色与职责)
  - [审核标准](#42-审核标准)
  - [反馈模板](#43-反馈模板)
- 🚨 [**常见提交事故与预防**](#5️⃣-常见提交事故与预防)
- 🎮 [**与引擎导入的衔接规范**](#6️⃣-与引擎导入的衔接规范)
- 📎 [**附录：审核流程时效要求**](#附录审核流程时效要求)
...
> 🚨 **[问题现象/事故描述]**：A 提交覆盖了 B 的修改，导致 B 数天工作白费
>
> 🔍 **[产生原因]**：未使用文件锁定 / 提交前未执行 Update 拉取最新版本
>
> 🛠️ **[解决方案]**：
> 1. PSD/MAX 等二进制文件强制使用 `exclusive lock`
> 2. 提交前必须先 `svn update` / `p4 sync`
>
> 🛡️ **[预防措施]**：配置版本管理工具的 **锁定策略**，二进制文件必须锁定后才能编辑

> ⚡ **APM 金句**：二进制文件不锁就是埋雷，出事只是时间问题。

### 📎 5.2 遗漏关联资产

> 🚨 **[问题现象/事故描述]**：提交了模型 FBX 但遗漏了贴图，引擎导入后白模
>
> 🔍 **[产生原因]**：手动选择文件提交，容易遗漏依赖文件
>
> 🛠️ **[解决方案]**：CI 脚本检查 FBX 引用的贴图是否同步提交
>
> 🛡️ **[预防措施]**：建立 **关联资产依赖检查** 机制，自动扫描 FBX 的贴图引用

### 🔗 5.3 引用路径断裂

> 🚨 **[问题现象/事故描述]**：引擎中资产引用找不到文件，界面/场景大面积丢失
>
> 🔍 **[产生原因]**：本地 rename 后重新 add，而非使用 `svn move` / `p4 move`
>
> 🛠️ **[解决方案]**：使用版本管理工具的 move 命令保留历史；CI 检查路径引用完整性
>
> 🛡️ **[预防措施]**：培训团队使用正确的重命名方式，配置 pre-commit hook 检查路径变更

### 💾 5.4 误提交大文件

> 🚨 **[问题现象/事故描述]**：提交了 2GB 的视频参考文件，仓库暴涨，团队 checkout 变慢
>
> 🔍 **[产生原因]**：`_Reference` 目录权限未限制，任何人都能提交任意大小文件
>
> 🛠️ **[解决方案]**：Pre-commit Hook 限制单文件大小（如 ≤ **200MB**）
>
> 🛡️ **[预防措施]**：配置 **文件大小限制 Hook** + `.svnignore` / `.p4ignore` 排除参考文件目录

> ⚡ **APM 金句**：仓库瘦身的最佳时机是「还没胖的时候」。
