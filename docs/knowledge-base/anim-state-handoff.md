...
> 📌 **场景说明**：动画片段命名
>
> ✅ **Do (正确示范)**：`AN_Luna_Idle_01_v02.fbx` / `AN_Luna_Attack_Heavy_v01.fbx` / `AN_Luna_Skill_Q_v03.fbx`
>
> ❌ **Don't (错误示范)**：`luna待机动画.fbx` / `Attack_1_final_final2.fbx` / `新建文件夹/动画.fbx`
...
> 🚨 **案例 1：骨骼不匹配**
>
> 🔴 **[高频]** 导入引擎后动画错乱、变形、T-Pose 残留。
>
> 🎬 **典型场景还原**
> - 美术：更新了骨骼结构（加了辅助骨骼），导出新动画
> - 程序：用旧骨骼导入新动画，导致全身错乱
>
> 🔍 **问题根因拆解**
> - 美术更新了骨骼结构但未同步给程序
...
> 4. 建立骨骼变更审批机制 — 改骨骼前必须群发通知 + 评估影响范围
...
>
> 🎬 **典型场景还原**
> - 美术：Attack 动画中 Root 骨骼有位移数据残留
> - 程序：引擎中未关闭 Root Motion，角色每次攻击后向前滑
>
> 🔍 **问题根因拆解**
> - Root Motion 配置不一致
> - 部分动画错误地包含了位移数据
...
>
> 🎬 **典型场景还原**
> - 程序：Idle→Run 条件为 `Speed>0`，Run→Idle 条件为 `Speed<1`
> - 结果：Speed=0.5 时两个状态每帧互跳
>
> 🔍 **问题根因拆解**
> - 转换条件互斥未处理，A→B 和 B→A 的条件同时满足
>
> 💡 **APM 破局思路 / 解决方案**
> 1. 添加 cooldown / 最小停留时间
> 2. 转换条件添加互斥逻辑
> 3. 使用 `ExitTime` 而非纯条件触发
> 4. 在状态转换条件表中逐条审查，确保无循环依赖
...
>
> 🔍 **问题根因拆解**
> - 可打断标记设置有误，错误地允许了不该打断的动画被打断
>
> 💡 **APM 破局思路 / 解决方案**
> 1. 攻击动画在 ComboWindow 内才可打断
> 2. 技能前摇不可打断，后摇可打断
> 3. 标注清楚每个动画的可打断窗口
> 4. 在状态转换条件表中增加"可打断"列标注
...
>
> ✅ **Do (正确示范)**：`[MOD][动画] AN_Luna_Attack01 调整打击帧从F15→F12，优化受击反馈 TASK-234`
>
> ❌ **Don't (错误示范)**：`update files` / `fix` / `修改了一些东西`
