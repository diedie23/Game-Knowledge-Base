...
- 📄 [**动画状态机文档化模板**](#1️⃣-动画状态机文档化模板)
  - [状态机总览表](#11-状态机总览表)
  - [状态转换条件表](#12-状态转换条件表)
  - [BlendSpace 参数定义](#13-blendspace-参数定义)
  - [状态机流程图](#14-状态机流程图)
- 📏 [**命名规范**](#2️⃣-命名规范)
  - [动画片段命名规则](#21-动画片段命名规则)
  - [动作类型前缀](#22-动作类型前缀)
  - [BlendSpace / Montage 命名](#23-blendspace--montage-命名)
- ⚙️ [**FBX 导出设置标准**](#3️⃣-fbx-导出设置标准)
  - [通用导出设置](#31-通用导出设置)
  - [骨骼导出设置](#32-骨骼导出设置)
  - [Root Motion 设置](#33-root-motion-设置)
  - [导出前检查清单](#34-导出前检查清单)
- ✅ [**引擎导入 Checklist**](#4️⃣-引擎导入-checklist)
  - [Unreal Engine](#41-unreal-engine)
  - [Unity](#42-unity)
  - [UE vs Unity 对照表](#43-ue-vs-unity-对照表)
- 📋 [**美术→程序交接 SOP**](#5️⃣-美术程序交接-sop)
  - [交接流程](#51-交接流程)
  - [交接清单](#52-交接清单)
  - [关键帧标注模板](#53-关键帧标注模板)
- 🚨 [**常见踩坑案例**](#6️⃣-常见踩坑案例)
...
> 📌 **[场景说明：动画片段命名]**
>
> ✅ **正确示范 (Do)**：`AN_Luna_Idle_01_v02.fbx` / `AN_Luna_Attack_Heavy_v01.fbx` / `AN_Luna_Skill_Q_v03.fbx`
>
> ❌ **错误示范 (Don't)**：`luna待机动画.fbx` / `Attack_1_final_final2.fbx` / `新建文件夹/动画.fbx`
...
> 🚨 **[问题现象/事故描述]**：导入引擎后动画错乱、变形、T-Pose 残留
>
> 🔍 **[产生原因]**：美术更新了骨骼结构但未同步给程序，多人协作时骨骼版本不一致
>
> 🛠️ **[解决方案]**：
> 1. 骨骼变更必须通知所有相关方
> 2. 使用版本号管理骨骼（如 `SK_Humanoid_v03`）
> 3. 引擎中统一使用共享 Skeleton Asset
>
> 🛡️ **[预防措施]**：建立骨骼变更审批机制 — 改骨骼前必须群发通知 + 评估影响范围

> ⚡ **APM 金句**：骨骼一变天下乱，改骨骼前先群发通知。

### 🔴 6.2 动画偏移

> 🚨 **[问题现象/事故描述]**：角色播放动画后位置偏移/不回原点
>
> 🔍 **[产生原因]**：Root Motion 配置不一致，部分动画错误地包含了位移数据
>
> 🛠️ **[解决方案]**：
> 1. 明确标注哪些动画需要 Root Motion
> 2. 不需要 Root Motion 的动画，Root 骨骼必须在原点不动
>
> 🛡️ **[预防措施]**：在交接文档中增加 Root Motion 标注列（参见 5.2 交接清单）

### 🟡 6.3 状态机死循环

> 🚨 **[问题现象/事故描述]**：两个状态互相跳转，动画抖动
>
> 🔍 **[产生原因]**：转换条件互斥未处理，A→B 和 B→A 的条件同时满足
>
> 🛠️ **[解决方案]**：
> 1. 添加 cooldown / 最小停留时间
> 2. 转换条件添加互斥逻辑
> 3. 使用 `ExitTime` 而非纯条件触发
>
> 🛡️ **[预防措施]**：在状态转换条件表中逐条审查，确保无循环依赖

### 🟡 6.4 过渡闪烁

> 🚨 **[问题现象/事故描述]**：状态切换瞬间出现闪烁/抖动
>
> 🔍 **[产生原因]**：混合时间设置不当，差异极大的 Pose 直接混合
>
> 🛠️ **[解决方案]**：
> 1. 攻击→待机：混合时间 **0.2~0.3s**
> 2. 受击：混合时间 **0.05s**（快速响应）
> 3. 避免两个差异极大的 Pose 直接混合
>
> 🛡️ **[预防措施]**：在状态机总览表中明确标注每个状态的混合时间（参见 1.1）

### 🟡 6.5 动画裁切

> 🚨 **[问题现象/事故描述]**：动画播到一半被打断，看起来不自然
>
> 🔍 **[产生原因]**：可打断标记设置有误，错误地允许了不该打断的动画被打断
>
> 🛠️ **[解决方案]**：
> 1. 攻击动画在 ComboWindow 内才可打断
> 2. 技能前摇不可打断，后摇可打断
> 3. 标注清楚每个动画的可打断窗口
>
> 🛡️ **[预防措施]**：在状态转换条件表中增加"可打断"列标注（参见 1.2）
...
> 📌 **[场景说明：Commit Message 规范]**
>
> ✅ **正确示范 (Do)**：`[MOD][动画] AN_Luna_Attack01 调整打击帧从F15→F12，优化受击反馈 TASK-234`
>
> ❌ **错误示范 (Don't)**：`update files` / `fix` / `修改了一些东西`
