...
| 状态名称 | 状态 ID | 动画片段 | 循环 | BlendSpace | 混合时间(入) | 混合时间(出) | 备注 |
|---------|---------|---------|------|------------|-------------|-------------|------|
...
示例：
AN_Luna_Idle_01_v02.fbx
AN_Luna_Attack_Heavy_v01.fbx
AN_Luna_Skill_Q_v03.fbx
```
...
```
美术完成动画 → 填写交接文档 → 提交到版本库
    ↓
程序/TA 接收 → 导入引擎 → 验证状态机 → 反馈问题
    ↓
问题修复 → 重新提交 → 二次验证 → 交接完成
```
...
**解法**：
- 骨骼变更必须通知所有相关方
- 使用版本号管理骨骼：`SK_Humanoid_v03`
- 引擎中统一使用共享 Skeleton Asset
...
**解法**：
- 明确标注哪些动画需要 Root Motion
- 不需要 Root Motion 的动画，Root 骨骼必须在原点不动
- 引擎端确认 Root Motion 设置与美术一致
...
```
[MOD][动画] AN_Luna_Attack01 调整打击帧从F15→F12，优化受击反馈 TASK-234
```
